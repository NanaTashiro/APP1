...
final_neural_predictions_2024_path = 'final_neural_predictions_2024.csv'
combined_neural_predictions_path = 'combined_neural_predictions.csv'
final_neural_predictions1_2024_path = 'final_neural_predictions1_2024.csv'
...
final_neural_predictions_2024 = pd.read_csv(final_neural_predictions_2024_path)
combined_neural_predictions = pd.read_csv(combined_neural_predictions_path)
final_neural_predictions1_2024 = pd.read_csv(final_neural_predictions1_2024_path)
...
# Streamlit title
st.title("Election Results Prediction Analysis")

# KNN Model
st.header("KNN Model of Predicted Election Results of Party List Vote in Percentage of All Electorates in Auckland")
...
st.header("Compare Actual and Predicted Votes (KNN Model)")
...
st.header("Predictions for 2024 (KNN Model)")
...
st.header("Neural Network Model of Predicted Election Results of Party List Vote in Percentage of All Electorates in Auckland")
...
# Function to create comparison DataFrame for a single year
def create_comparison_df_nn(year, electorate):
...
# Function to plot comparison for a single year
def plot_comparison_nn(comparison_df, year, electorate):
    if comparison_df.empty:
        st.write(f"No common electorates for {year}. Skipping plot.")
        return
...
# User inputs for comparison
st.header("Compare Actual and Predicted Votes (Neural Network Model)")
year_nn = st.selectbox("Select Year (Neural Network)", [2017, 2020, 2023])
electorate_nn = st.selectbox("Select Electorate (Neural Network)", combined_result_list['Electorate'].unique())

comparison_df_nn = create_comparison_df_nn(year_nn, electorate_nn)
plot_comparison_nn(comparison_df_nn, year_nn, electorate_nn)

# Function to plot 2024 predictions with color by party name as bar charts and add results number
def plot_predictions_2024_nn(predictions_df, electorate):
    predictions = predictions_df[predictions_df['Electorate'] == electorate]
    if predictions.empty:
        st.write(f"No data available for {electorate} in 2024.")
        return
    ...
    ax.set_title(f'Predicted Votes for {electorate} in 2024 (Neural Network)')
    ax.set_xlabel('Party')
    ax.set_ylabel('Votes (%)')
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right')
    st.pyplot(fig)
...
# User input for 2024 predictions
st.header("Predictions for 2024 (Neural Network Model)")
electorate_2024_nn = st.selectbox("Select Electorate for 2024 (Neural Network)", final_neural_predictions1_2024['Electorate'].unique())
plot_predictions_2024_nn(final_neural_predictions1_2024, electorate_2024_nn)
